function [bancodeltaspsnormalizado] = rcrpornivel (bancopedfgeneralcell, nivelwavelet)
...
delta=zeros;
    while (Linha<=ordem(1,1))
        ...
        L=1;
        tamanho=round(tamanho/nivelwavelet); %editado
...
while (Coluna<=ordem2(1,2))
    ...
    while (C<=ordem(1,2))
        if (size(banco{1,C})==size(fic)) %o maior delta p é 37, mas excede a dist em x
